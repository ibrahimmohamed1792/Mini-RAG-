...
class DataChunkModel(BaseDataModel):
    ...
    async def init_collection(self):
        ...
        if DataBaseEnum.COLLECTION_CHUNK_NAME.value not in all_collectinos:
            self.collection=self.db_client[DataBaseEnum.COLLECTION_CHUNK_NAME.value]
    ...
    async def insert_many_chunks(self,chunks:list,batch_size:int=100):
        for i in range(0,len(chunks),batch_size):
            ...
            await self.collection.bulk_write(Opreations)
    ...
    async def get_project_chunks(self,project_id:ObjectId,page_no:int=1,page_size:int=50):
        ...
        records = await self.collection.find({
                    ...
                    (max(1, page_no) - 1) * page_size).limit(page_size).to_list(length=page_size)
